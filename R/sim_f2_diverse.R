...
for (i in 1:10) {
    ...
    system(paste("mkdir simulations/f2_diverse", i, sep = ""))
    setwd(paste(base_dir,
                "/simulations/f2_diverse",
                i, sep = ""))
    
    mapping_population <- setup_population("f2_diverse")
    ...
    snps_dense <- pullSnpGeno(f2, 1)
    snps_sparse <- pullSnpGeno(f2, 2)
    ...
    recoded_sparse <- recode_genotypes_diverse(snps_sparse, parental_snps_sparse)
